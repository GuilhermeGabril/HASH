#TABELA HASH - MÉTODO DE COLISÃO: ENCADEAMENTO EXTERNO - UTILIZANDO OS 3 ÚLTIMOS DÍGITOS DA CHAVE NA INSERÇÃO
...
class TabelaHash:
     ...
     def buscar_elemento(self, elemento):  # método para buscar um elemento
        chave = elemento % 1000  # utilizará apenas os 3 últimos dígitos da chave
        resto = chave % 9  # calcula o resto
     ...
     def criar_tabela_excel(self): #método para criação da tabela excel
        ...
        df.to_excel("tabela_hash_1.xlsx", index=False)
...
print('A tabela HASH está no arquivo: tabela_hash_1.xlsx')